...
export default function SeachForm(props) {
  ...
  const handleCategoryChange = (e) => {
    ...
    if (checked)
      if (category.length > 0) setCategory([...category, noValue]);
      else setCategory([noValue]);
    else {
  ...
  const handleBrandChange = (e) => {
    ...
    const noValue = parseInt(value);
    if (checked)
      if (brand.length > 0) setBrand([...brand, noValue]);
      else setBrand([noValue]);
    else {
